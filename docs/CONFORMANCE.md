...
The nested-ref matrix is generated by `go/schemapb/golden_nested_ref_test.go`.
All four runners read the same typed `StructValue` inputs from the fixture;
these cases do not maintain separate native input copies. Successful cases
compare the complete baked `StructValue`, exercise Resolve followed by Bake,
and re-bake the wire snapshot. Invalid cases compare the complete
`ValidationResult` and require the absence of a baked snapshot. The matrix
