...
func CanonicalValue(f *Schema_Field, x any) (*Value, error) {
	...
	case *Schema_Field_Computed_:
		// Computed values canonicalise by the declared result type.
		return canonicalResult(f.GetComputed().GetResult(), x)
	default:
		// OneOf / Ref values canonicalise structurally (variant/def schemas are
		// resolved by the engine, which canonicalises through them instead).
		return FromGo(x)
	}
}

// canonicalStruct canonicalises a native map against a schema's declared
...
// canonicalResult canonicalises a computed result by its declared ResultType.
